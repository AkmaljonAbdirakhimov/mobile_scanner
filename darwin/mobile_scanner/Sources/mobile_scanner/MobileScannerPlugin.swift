...
public class MobileScannerPlugin: NSObject, FlutterPlugin, FlutterStreamHandler, FlutterTexture, AVCaptureVideoDataOutputSampleBufferDelegate {
    ...
    // Credit card detection properties
    var detectionType: String = "barcode" // "barcode" or "creditCard"
    var creditCardConfidenceThreshold: Float = 0.7
    ...
    public static func register(with registrar: FlutterPluginRegistrar) {
        ...
        let method = FlutterMethodChannel(name:
                                            "uz.akmaljonabdirakhimovmobile_scanner/scanner/method", binaryMessenger: messenger)
        let event = FlutterEventChannel(name:
                                            "uz.akmaljonabdirakhimovmobile_scanner/scanner/event", binaryMessenger: messenger)
        ...
        let orientationEvent = FlutterEventChannel(name:
                                            "uz.akmaljonabdirakhimovmobile_scanner/scanner/deviceOrientation", binaryMessenger: messenger)
    ...
    public func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
        ...
        case "analyzeImageForCreditCards":
            analyzeImageForCreditCards(call, result)
        default:
    ...
    public func captureOutput(_ output: AVCaptureOutput, didOutput sampleBuffer: CMSampleBuffer, from connection: AVCaptureConnection) {
        ...
        if ((detectionSpeed == DetectionSpeed.normal || detectionSpeed == DetectionSpeed.noDuplicates) && eligibleForScan || detectionSpeed == DetectionSpeed.unrestricted) {
            ...
            DispatchQueue.global(qos: .userInitiated).async { [weak self] in
                ...
                else if self?.detectionType == "creditCard" {
                    // Credit card detection requires iOS 13.0+
                    if #available(iOS 13.0, macOS 10.15, *) {
                        do {
                            let textRecognitionRequest: VNRecognizeTextRequest = VNRecognizeTextRequest(completionHandler: { [weak self] (request, error) in
                                self?.imagesCurrentlyBeingProcessed = false
                                ...
                                // Extract credit card information from text observations
                                let creditCards = self?.extractCreditCards(from: observations, image: cgImage) ?? []
                            ...
                            textRecognitionRequest.customWords = ["VISA", "MASTERCARD", "AMEX", "DISCOVER", "JCB", "DINERS", "CUP", "MAESTRO", "ELECTRON", "UNIONPAY"]

                            // Set the region of interest to match scanWindow
                            if let scanWindow = self?.scanWindow {
                                textRecognitionRequest.regionOfInterest = scanWindow
                            }
                        ...
                        // For iOS versions below 13.0, fall back to barcode detection
                        DispatchQueue.main.async {
                            self?.sink?(FlutterError(
                                code: MobileScannerErrorCodes.UNSUPPORTED_OPERATION_ERROR,
                                message: "Credit card detection requires iOS 13.0 or later",
                                details: nil))
                        }
                    }
                }
    ...
    func start(_ call: FlutterMethodCall, _ result: @escaping FlutterResult) {
        ...
        // Credit card detection parameters
        let detectionTypeInt: Int = argReader.int(key: "detectionType") ?? 0
        detectionType = detectionTypeInt == 1 ? "creditCard" : "barcode"
        creditCardConfidenceThreshold = Float(argReader.double(key: "creditCardConfidenceThreshold") ?? 0.7)
    ...
    func analyzeImageForCreditCards(_ call: FlutterMethodCall, _ result: @escaping FlutterResult) {
        // The iOS Simulator cannot use some of the GPU features that are required for the Vision API.
        // Thus analyzing images is not supported on the iOS Simulator.
        //
        // See https://forums.developer.apple.com/forums/thread/696714
        ...
        if #available(iOS 13.0, macOS 10.15, *) {
            ...
            do {
                let textRecognitionRequest: VNRecognizeTextRequest = VNRecognizeTextRequest(completionHandler: { [weak self] (request, error) in
                    ...
                    let creditCardText = observations.compactMap({ $0.topCandidates(1).first?.string }).joined(separator: " ")
                    ...
                    DispatchQueue.main.async {
                        result([
                            ...
                            "data": [
                                "text": creditCardText,
                                "confidence": 1.0, // Assuming 100% confidence for now
                            ],
                        ])
                    }
                })
            ...
            // For iOS versions below 13.0, return an error
            result(FlutterError(
                code: MobileScannerErrorCodes.UNSUPPORTED_OPERATION_ERROR,
                message: "Credit card detection requires iOS 13.0 or later",
                details: nil
...
extension MobileScannerPlugin {
    /// Extract credit card information from text observations
    @available(iOS 13.0, macOS 10.15, *)
    func extractCreditCards(from observations: [VNRecognizedTextObservation], image: CGImage?) -> [[String: Any]] {
        ...
        for observation in observations {
            ...
            if confidence < creditCardConfidenceThreshold {
                continue
            }
            
            // Extract credit card information using regex patterns
            let cardNumber = extractCardNumber(from: text)
            let expiryDate = extractExpiryDate(from: text)
            let cardholderName = extractCardholderName(from: text)
            let cvv = extractCVV(from: text)
            
            // Create credit card if we found any valid data
            if cardNumber != nil || expiryDate != nil || cardholderName != nil {
                let corners = observation.boundingBox
                let size = observation.boundingBox
                ...
                let creditCard: [String: Any] = [
                    ...
                    "cvv": cvv,
                    "corners": [
                        ["x": corners.minX, "y": corners.minY],
                        ["x": corners.maxX, "y": corners.minY],
                        ["x": corners.maxX, "y": corners.maxY],
                        ["x": corners.minX, "y": corners.maxY]
                    ],
                    "size": [
                        "width": size.width,
                        "height": size.height
    ...
    /// Extract credit card number from text using regex
    private func extractCardNumber(from text: String) -> String? {
        ...
        let cardNumber = String(text[Range(match.range, in: text)!])
        
        // Remove spaces and dashes
        let cleanedNumber = cardNumber.replacingOccurrences(of: "[\\s-]", with: "", options: .regularExpression)
        
        // Validate using Luhn algorithm
        return isValidLuhn(cleanedNumber) ? cleanedNumber : nil
    }
    
    /// Extract expiry date from text using regex
    private func extractExpiryDate(from text: String) -> String? {
        let pattern = #"\b(0[1-9]|1[0-2])[/-]([0-9]{2}|[0-9]{4})\b"#
        let regex = try? NSRegularExpression(pattern: pattern)
        let range = NSRange(location: 0, length: text.utf16.count)
        
        guard let match = regex?.firstMatch(in: text, range: range) else { return nil }
        let expiry = String(text[Range(match.range, in: text)!])
    ...
    /// Extract cardholder name from text using regex
    ...
    /// Extract CVV from text using regex
    private func extractCVV(from text: String) -> String? {
    ...
    private func isValidLuhn(_ cardNumber: String) -> Bool {
        ...
        let digits = cardNumber.replacingOccurrences(of: "\\D", with: "", options: .regularExpression)
        ...
        for i in stride(from: digits.count - 1, through: 0, by: -1) {
            let digit = Int(String(digits[digits.index(digits.startIndex, offsetBy: i)]))!
