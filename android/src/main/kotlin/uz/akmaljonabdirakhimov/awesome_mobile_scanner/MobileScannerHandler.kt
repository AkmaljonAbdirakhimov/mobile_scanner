...
import uz.akmaljonabdirakhimov.awesome_mobile_scanner.objects.BarcodeFormats
...
//            ))
    ...
    private val callback: MobileScannerCallback = { barcodes: List<Map<String, Any?>>, creditCards: List<Map<String, Any?>>, image: ByteArray?, width: Int?, height: Int? ->
        // Publish barcode event if barcodes are detected
        ...
        // Publish credit card event if credit cards are detected
    ...
    init {
        methodChannel = MethodChannel(binaryMessenger,
            "uz.akmaljonabdirakhimov.awesome_mobile_scanner/scanner/method")
    ...
    override fun onMethodCall(call: MethodCall, result: MethodChannel.Result) {
        when (call.method) {
            ...
            "analyzeImageForCreditCards" -> analyzeImageForCreditCards(call, result)
    ...
    private fun start(call: MethodCall, result: MethodChannel.Result) {
        ...
        val detectionType: Int = call.argument<Int>("detectionType") ?: 0
        val creditCardConfidenceThreshold: Double = call.argument<Double>("creditCardConfidenceThreshold") ?: 0.7
    ...
    private fun analyzeImage(call: MethodCall, result: MethodChannel.Result) {
        ...
        val formats: List<Int>? = call.argument<List<Int>>("formats")
        val barcodeScannerOptions: BarcodeScannerOptions? = buildBarcodeScannerOptions(formats, false)
    ...
    private fun analyzeImageForCreditCards(call: MethodCall, result: MethodChannel.Result) {
        val filePath: String = call.argument<String>("filePath") ?: ""
        val confidenceThreshold: Double = call.argument<Double>("confidenceThreshold") ?: 0.7
