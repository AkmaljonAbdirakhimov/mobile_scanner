...
import uz.akmaljonabdirakhimov.awesome_mobile_scanner.objects.DetectionSpeed
...
import uz.akmaljonabdirakhimov.awesome_mobile_scanner.utils.CreditCardExtractor
import uz.akmaljonabdirakhimov.awesome_mobile_scanner.utils.YuvToRgbConverter
import uz.akmaljonabdirakhimov.awesome_mobile_scanner.utils.serialize
...
class MobileScanner(
    ...
    private var detectionType: String = "barcode" // "barcode" or "creditCard"
    private var creditCardConfidenceThreshold: Double = 0.7
    ...
    val captureOutput = ImageAnalysis.Analyzer { imageProxy -> // YUV_420_888 format
                ...
                it.process(inputImage).addOnSuccessListener { barcodes ->
                    ...
                    if (!returnImage) {
                        mobileScannerCallback(
                            barcodeMap,
                            emptyList(), // Empty credit card list
                            null,
        ...
        else if (detectionType == "creditCard") {
            textRecognizer?.let {
                it.process(inputImage).addOnSuccessListener { text ->
                    val creditCards = CreditCardExtractor.extractCreditCards(text, creditCardConfidenceThreshold)
                    
                    if (creditCards.isNotEmpty()) {
                        val creditCardMap = creditCards.map { it.toMap() }
                        val portrait = (camera?.cameraInfo?.sensorRotationDegrees ?: 0) % 180 == 0
    ...
    fun start(
        ...
        invertImage: Boolean,
        detectionType: Int,
        creditCardConfidenceThreshold: Double,
    ) {
        this.detectionSpeed = detectionSpeed
        this.detectionTimeout = detectionTimeout
        this.returnImage = returnImage
        this.invertImage = invertImage
        this.detectionType = if (detectionType == 1) "creditCard" else "barcode"
        this.creditCardConfidenceThreshold = creditCardConfidenceThreshold
...
//    }
    ...
    /**
     * Analyze a local image file for credit cards.
     */
    fun analyzeImageForCreditCards(
        imagePath: String,
        ...
        try {
            inputImage = InputImage.fromFilePath(activity, Uri.parse(imagePath))
        } catch (error: IOException) {
            onError(MobileScannerErrorCodes.ANALYZE_IMAGE_NO_VALID_IMAGE_ERROR_MESSAGE)
            return
        }

        // Use a short lived text recognizer instance
        val textRecognizer = TextRecognition.getClient(TextRecognizerOptions.DEFAULT_OPTIONS)

        textRecognizer.process(inputImage).addOnSuccessListener { text ->
            val creditCards = CreditCardExtractor.extractCreditCards(text, confidenceThreshold)
            val creditCardMap = creditCards.map { it.toMap() }

            onSuccess(creditCardMap)
        }.addOnFailureListener { e ->
            onError(e.localizedMessage ?: e.toString())
        }.addOnCompleteListener {
    ...
    fun dispose() {
        ...
        // Close text recognizer if it exists
        textRecognizer?.close()
        textRecognizer = null
