...
class MobileScanner extends StatefulWidget {
  ...
  const MobileScanner({
    this.controller,
    this.onDetect,
    this.onCreditCardDetect,
    this.onDetectError = _onDetectErrorHandler,
    this.fit = BoxFit.cover,
    this.errorBuilder,
    this.overlayBuilder,
    this.creditCardOverlayBuilder,
  ...
  /// The function that signals when new credit cards were detected by the
  /// [controller].
  ///
  /// To handle both [CreditCardCapture]s and [MobileScannerException]s,
  /// use the [MobileScannerController.creditCards] stream directly (recommended),
  /// or provide a function to [onDetectError].
  final void Function(CreditCardCapture creditCards)? onCreditCardDetect;
  ...
  /// The builder for the credit card overlay above the camera preview.
  ///
  /// The resulting widget can be combined with the [scanWindow] rectangle
  /// to create a cutout for the camera preview.
  ///
  /// The overlay is only displayed when the camera preview is visible.
  final Widget Function(BuildContext, List<CreditCard>)? creditCardOverlayBuilder;
...
class _MobileScannerState extends State<MobileScanner> with WidgetsBindingObserver {
      ...
      builder: (BuildContext context, MobileScannerState value, _) {
        ...
        return LayoutBuilder(
          builder: (context, constraints) {
            ...
            return Stack(alignment: Alignment.center, children: <Widget>[scannerWidget, overlay]);
          },
        );
      },
    );
  ...
  StreamSubscription<CreditCardCapture>? _creditCardSubscription;
  ...
  Future<void> initMobileScanner() async {
    ...
    if (widget.onCreditCardDetect != null) {
      _creditCardSubscription = controller.creditCards.listen(
        widget.onCreditCardDetect,
  ...
  Future<void> disposeMobileScanner() async {
    ...
    await _creditCardSubscription?.cancel();
    _subscription = null;
    _creditCardSubscription = null;
