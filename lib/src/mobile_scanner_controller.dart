...
class MobileScannerController extends ValueNotifier<MobileScannerState> {
  ...
  MobileScannerController({
    ...
    this.creditCardConfidenceThreshold = 0.7,
    ...
    this.autoZoom = false,
       ...
       assert(facing != CameraFacing.unknown, 'CameraFacing.unknown is not a valid camera direction.'),
       assert(
         creditCardConfidenceThreshold >= 0.0 && creditCardConfidenceThreshold <= 1.0,
         'Credit card confidence threshold must be between 0.0 and 1.0.',
       ),
  ...
  ///
  /// Defaults to [DetectionType.barcode].
  final DetectionType detectionType;

  /// The confidence threshold for credit card detection (0.0 to 1.0).
  ///
  /// Only credit cards with confidence scores above this threshold will be
  /// reported. Defaults to 0.7.
  final double creditCardConfidenceThreshold;
  ...
  /// The internal credit card controller, that listens for detected credit cards.
  final StreamController<CreditCardCapture> _creditCardsController = StreamController.broadcast();
  ...
  /// Get the stream of scanned credit cards.
  ///
  /// If an error occurred during the detection of a credit card,
  /// a [MobileScannerException] error is emitted to the stream.
  Stream<CreditCardCapture> get creditCards => _creditCardsController.stream;

  StreamSubscription<BarcodeCapture?>? _barcodesSubscription;
  StreamSubscription<CreditCardCapture?>? _creditCardsSubscription;
  ...
  void _setupListeners() {
    ...
    _creditCardsSubscription = MobileScannerPlatform.instance.creditCardsStream.listen(
      (CreditCardCapture? creditCard) {
        if (_creditCardsController.isClosed || creditCard == null) {
          return;
        }

        _creditCardsController.add(creditCard);
      },
      onError: (Object error) {
        if (_creditCardsController.isClosed) {
          return;
        }

        _creditCardsController.addError(error);
      },
      // Errors are handled gracefully by forwarding them.
      cancelOnError: false,
    );
    ...
    if (MobileScannerPlatform.instance case final MethodChannelMobileScanner implementation
      ...
      _deviceOrientationSubscription = implementation.deviceOrientationChangedStream.listen((
  ...
  void _throwIfNotInitialized() {
    if (!value.isInitialized) {
      throw MobileScannerException(
        ...
        errorDetails: MobileScannerErrorDetails(message: MobileScannerErrorCode.controllerUninitialized.message),
      );
    }
    ...
    if (_isDisposed) {
      throw MobileScannerException(
        ...
        errorDetails: MobileScannerErrorDetails(message: MobileScannerErrorCode.controllerDisposed.message),
      );
    }
  }
  ...
  Future<void> start({CameraFacing? cameraDirection}) async {
    ...
    if (!_isAttachedCompleter.isCompleted) {
      ...
      await _isAttachedCompleter.future.timeout(const Duration(milliseconds: 500)).catchError((
        Object error,
    ...
    if (cameraDirection == CameraFacing.unknown) {
      throw const MobileScannerException(
        ...
        errorDetails: MobileScannerErrorDetails(message: 'CameraFacing.unknown is not a valid camera direction.'),
      );
    }
    ...
    if (value.isStarting) {
      throw MobileScannerException(
        ...
        errorDetails: MobileScannerErrorDetails(message: MobileScannerErrorCode.controllerInitializing.message),
      );
    }
    ...
    final StartOptions options = StartOptions(
      ...
      creditCardConfidenceThreshold: creditCardConfidenceThreshold,
      formats: formats,
    ...
    try {
      ...
      final MobileScannerViewAttributes viewAttributes = await MobileScannerPlatform.instance.start(options);
  ...
  Future<void> switchCamera() async {
    ...
    if (cameraDirection == CameraFacing.unknown || cameraDirection == CameraFacing.external) {
      return;
    }
